...
class SubmitWorker(BaseWorker):
    ...
    async def _submit_to_platform(self, bounty: Bounty, output: str) -> bool:
        ...
        if bounty.source == "github" and bounty.url and "github.com" in bounty.url:
            return await self._submit_github_comment(bounty, output)
        logger.warning(f"[submit] No real submission for source: {bounty.source}")
        return False

    async def _submit_github_comment(self, bounty: Bounty, output: str) -> bool:
        """Στέλνει comment στο GitHub issue με το AI-generated work."""
        import os, httpx
        token = os.environ.get("GITHUB_TOKEN", "")
        if not token:
            logger.error("[submit] No GITHUB_TOKEN found")
            return False
        # Parse owner/repo/issue από URL
        # URL format: https://github.com/owner/repo/issues/123
        parts = bounty.url.rstrip("/").split("/")
        if len(parts) < 7:
            logger.error(f"[submit] Cannot parse GitHub URL: {bounty.url}")
            return False
        owner, repo, issue_number = parts[-4], parts[-3], parts[-1]
        api_url = f"https://api.github.com/repos/{owner}/{repo}/issues/{issue_number}/comments"
        comment = f"""## 🤖 Bounty Hunter Agent — Work Submission

{output}

---
*Submitted automatically by Bounty Hunter Agent*
"""
        headers = {
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if resp.status_code == 201:
                    logger.info(f"[submit] ✅ GitHub comment posted: {resp.json().get('html_url')}")
                    return True
                else:
                    logger.error(f"[submit] GitHub API error {resp.status_code}: {resp.text[:200]}")
                    return False
        except Exception as e:
            logger.error(f"[submit] GitHub post error: {e}")
            return False
